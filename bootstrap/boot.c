/**
 * Code to support the boot process (displaying the boot screen, loading the first program, etc.)
 */
...
void boot_display_banner() {
    const char *badge = "\e[91m\x0b\x0c\e[93m\x0b\x0c\e[92m\x0b\x0c\e[94m\x0b\x0c\e[95m\x0b\x0c\e[97m";

    char * message = "Foenix Toolbox v01.02.0000";
    int message_len = strlen(message);

    printf("\e[3J\e[H");
    printf("    %s     _________   ___   ___________ ____  __ _____ \n", badge);
    printf("   %s     / ____/   | |__ \\ / ____/ ___// __ \\/ //_/__ \\\n", badge);
    printf("  %s     / /_  / /| | __/ //___ \\/ __ \\/ / / / ,<  __/ /\n", badge);
    printf(" %s     / __/ / ___ |/ __/____/ / /_/ / /_/ / /| |/ __/ \n", badge);
    printf("%s     /_/   /_/  |_/____/_____/\\____/\\____/_/ |_/____/ \n", badge);

    printf("\e[%d;%dH%s", 128 - message_len, 5, message);
    printf("\e[1;6H");
}

static void frame_draw(p_rect frame) {
	t_rect region;
	short pos;
	...
	// Drop the top
	buffer[0] = 0xda;
	for (pos = 1; pos < frame->size.width; pos++) {
		buffer[pos] = 0xc4;
	}
	buffer[frame->size.width] = 0xbf;
	buffer[frame->size.width + 1] = 0;
	printf("\e[%d;%dH%s", frame->origin.x + 1, frame->origin.y + 1, buffer);
	...
	buffer[frame->size.width] = 0xb3;
	buffer[frame->size.width + 1] = 0;
	for (pos = 1; pos < frame->size.height; pos++) {
		printf("\e[%d;%dH%s", frame->origin.x + 1, frame->origin.y + 1 + pos, buffer);
	}
	...
	buffer[frame->size.width] = 0xd9;
	buffer[frame->size.width + 1] = 0;
	printf("\e[%d;%dH%s", frame->origin.x + 1, frame->origin.y + frame->size.height + 1, buffer);

	// Set region to the area inside the frame, and clear it
	region.origin.x = frame->origin.x + 1;
	region.origin.y = frame->origin.y + 1;
	region.size.width = frame->size.width - 2;
	region.size.height = frame->size.height - 2;
	txt_set_region(0, &region);
	txt_clear(0, 2);
}
...
/**
 * Return the name of the boot source device
 * 
 * @param device the code for the bootable device
 * @return the name of the bootable device
 */
...
// /**
//  * Boot from the given bootable device
//  * 
//  * @param device the code of the bootable device
//  * @param boot_record pointer to the boot record for the device
//  */
...
void boot_screen() {
	...
	log_frame.origin.x = 10;
	log_frame.origin.y = 10;
	log_frame.size.width = 108;
	log_frame.size.height = 76; 
	frame_draw(&log_frame);

	printf("\e[HFoenix Toolbox Bootstrap...\n");
}
