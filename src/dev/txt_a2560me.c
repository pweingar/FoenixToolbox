...
unsigned char a2560me_enable_set_sizes;    /* Flag to enable set_sizes to actually do its computation */
t_txt_capabilities a2560me_caps;           /* The capabilities of Channel B */
t_extent a2560me_resolutions[6];           /* The list of display resolutions */
t_extent a2560me_fonts[1];                 /* The list of font resolutions */
t_rect a2560me_region;                     /* The current region */
t_point a2560me_cursor;                    /* The current cursor position */
t_extent a2560me_resolution;               /* The current display resolution */
t_extent a2560me_font_size;                /* The current font size */
t_extent a2560me_max_size;                 /* The size of the screen in characters (without border removed) */
t_extent a2560me_visible_size;             /* The size of the visible screen in characters (with border removed) */
short a2560me_border_width;                /* Width of the border on one side */
short a2560me_border_height;               /* Height of the border on one side */
unsigned char a2560me_color;               /* The current color */
 ...
 */
const p_txt_capabilities txt_a2560me_get_capabilities() {
    return &a2560me_caps;
}
 ...
 */
void txt_a2560me_set_sizes() {
    if (a2560me_enable_set_sizes) {
         ...
         */
        a2560me_max_size.width = a2560me_resolution.width / a2560me_font_size.width;
        a2560me_max_size.height = a2560me_resolution.height / a2560me_font_size.height;

        /*
         * Calculate the characters that are visible in whole or in part
         */
        short border_width = (2 * a2560me_border_width) / a2560me_font_size.width;
        short border_height = (2 * a2560me_border_height) / a2560me_font_size.height;
        a2560me_visible_size.width = a2560me_max_size.width - border_width;
        a2560me_visible_size.height = a2560me_max_size.height - border_height;
    }
}
 ...
 */
void txt_a2560me_get_sizes(p_extent text_size, p_extent pixel_size) {
    if (text_size) {
        text_size->width = a2560me_visible_size.width;
        text_size->height = a2560me_visible_size.height;
    }

    if (pixel_size) {
        pixel_size->width = a2560me_resolution.width;
        pixel_size->height = a2560me_resolution.height;
    }
}
 ...
 */
short txt_a2560me_set_mode(short mode) {
 ...
 */
short txt_a2560me_set_resolution(short width, short height) {
    int i;

    // TODO: set the size based on the DIP... or move this elsewhere?
    ...
    for (i = 0; i < a2560me_caps.resolution_count; i++) {
        if ((a2560me_caps.resolutions[i].width == width) && (a2560me_caps.resolutions[i].height == height)) {
            msr_shadow_b &= ~(VKY3_B_DOUBLE | VKY3_B_MODE0 | VKY3_B_MODE1);

            // For the moment, we leave the clock and the MODE alone

            // Update the kernel variables
            a2560me_resolution.width = width;
            a2560me_resolution.height = height;

            // Recalculate the size of the screen
            txt_a2560me_set_sizes();

            // TODO: do we need to kick the PLL like we did on older vicky?
 ...
 */
void txt_a2560me_set_border(short width, short height) {
    if ((width > 0) || (height > 0)) {
        a2560me_border_width = width;
        a2560me_border_height = height;
        *VKY3_B_BCR = (height & 0x3f) << 16 | (width & 0x3f) << 8 | VKY3_B_BCR_ENABLE;
        ...
        a2560me_border_width = 0;
        a2560me_border_height = 0;
        *VKY3_B_BCR = 0;
 ...
 */
void txt_a2560me_set_border_color(unsigned char red, unsigned char green, unsigned char blue) {
...
short txt_a2560me_set_font(short width, short height, const unsigned char * data) {
    if ((width == 8) && (height == 16)) {
        ...
        a2560me_font_size.width = width;
        a2560me_font_size.height = height;
 ...
 */
void txt_a2560me_set_cursor(short enable, short rate, char c) {
    *VKY3_B_CCR = ((a2560me_color & 0xff) << 24) | ((c & 0xff) << 16) | ((rate & 0x03) << 1) | (enable & 0x01);
}
 ...
 * @param enable 0 to hide, any other number to make visible
 */
 void txt_a2560me_set_cursor_visible(short enable) {
     if (enable) {
         *VKY3_B_CCR |= 0x01;
     } else {
         *VKY3_B_CCR &= ~0x01;
 ...
 */
 short txt_a2560me_set_region(p_rect region) {
     if ((region->size.width == 0) || (region->size.height == 0)) {
         /* Set the region to the default (full screen) */
         a2560me_region.origin.x = 0;
         a2560me_region.origin.y = 0;
         a2560me_region.size.width = a2560me_visible_size.width;
         a2560me_region.size.height = a2560me_visible_size.height;

     } else {
         a2560me_region.origin.x = region->origin.x;
         a2560me_region.origin.y = region->origin.y;
         a2560me_region.size.width = region->size.width;
         a2560me_region.size.height = region->size.height;
     }
 ...
 * @return 0 on success, any other number means the region was invalid
 */
short txt_a2560me_get_region(p_rect region) {
    region->origin.x = a2560me_region.origin.x;
    region->origin.y = a2560me_region.origin.y;
    region->size.width = a2560me_region.size.width;
    region->size.height = a2560me_region.size.height;
 ...
 */
short txt_a2560me_set_color(unsigned char foreground, unsigned char background) {
    a2560me_color = ((foreground & 0x0f) << 4) | (background & 0x0f);
    return 0;
}
 ...
 */
short txt_a2560me_get_color(unsigned char * foreground, unsigned char * background) {
    *foreground = (a2560me_color & 0xf0) >> 4;
    *background = a2560me_color & 0x0f;
    return 0;
}
 ...
 */
void txt_a2560me_scroll(short horizontal, short vertical) {
    ...
    if (vertical >= 0) {
        y0 = a2560me_region.origin.y;
        y1 = y0 + vertical;
        y3 = a2560me_region.origin.y + a2560me_region.size.height;
        y2 = y3 - vertical;
        dy = 1;
    } else {
        y0 = a2560me_region.origin.y + a2560me_region.size.height - 1;
        y1 = y0 + vertical;
        y3 = a2560me_region.origin.y - 1;
        y2 = y3 - vertical;
        dy = -1;
    }
    ...
    if (horizontal >= 0) {
        x0 = a2560me_region.origin.x;
        x1 = x0 + horizontal;
        x3 = a2560me_region.origin.x + a2560me_region.size.width;
        x2 = x3 - horizontal;
        dx = 1;
    } else {
        x0 = a2560me_region.origin.x + a2560me_region.size.width - 1;
        x1 = x0 + horizontal;
        x3 = a2560me_region.origin.x - 1;
        x2 = x3 - horizontal;
        dx = -1;
    }
    ...
    for (y = y0; y != y2; y += dy) {
        int row_dst = y * a2560me_max_size.width;
        int row_src = (y + vertical) * a2560me_max_size.width;
    ...
    if (horizontal != 0) {
        for (y = y0; y != y3; y += dy) {
            ...
            for (x = x2; x != x3; x += dx) {
                ...
                VKY3_B_COLOR_MATRIX[row_dst + x] = a2560me_color;
            }
        }
    }
 ...
 */
void txt_a2560me_fill(char c) {
    int x;
    int y;

    for (y = 0; y < a2560me_region.size.height; y++) {
        int offset_row = (a2560me_region.origin.y + y) * a2560me_max_size.width;
        for (x = 0; x < a2560me_region.size.width; x++) {
            int offset = offset_row + a2560me_region.origin.x + x;
            VKY3_B_TEXT_MATRIX[offset] = c;
            VKY3_B_COLOR_MATRIX[offset] = a2560me_color;
        }
    }
}
 ...
 */
void txt_a2560me_set_xy(short x, short y) {
    /* Make sure X is within range for the current region... "print" a newline if not */
    if (x < 0) {
        x = 0;
    } else if (x >= a2560me_region.size.width) {
        x = 0;
        y++;
    }
    ...
    if (y < 0) {
        y = 0;
    } else if (y >= a2560me_region.size.height) {
        txt_a2560me_scroll(0, y - a2560me_region.size.height + 1);
        y = a2560me_region.size.height - 1;
    }
    ...
    /* Set register */
    *VKY3_B_CPR = (((a2560me_region.origin.y + y) & 0xffff) << 16) | ((a2560me_region.origin.x + x) & 0xffff);
}
 ...
 */
void txt_a2560me_get_xy(p_point position) {
    position->x = a2560me_cursor.x;
    position->y = a2560me_cursor.y;
}
 ...
 */
void txt_a2560me_put(char c) {
    short x;
    short y;
    unsigned int offset;

    x = a2560me_region.origin.x + a2560me_cursor.x;
    y = a2560me_region.origin.y + a2560me_cursor.y;
    offset = y * a2560me_max_size.width + x;
    VKY3_B_TEXT_MATRIX[offset] = c;
    VKY3_B_COLOR_MATRIX[offset] = a2560me_color;

    txt_a2560me_set_xy(a2560me_cursor.x + 1, a2560me_cursor.y);
}

/**
 * Initialize the screen
 */
void txt_a2560me_init() {
    char buffer[255];
    t_rect region;
    int i;
    ...
    a2560me_resolution.width = 0;
    a2560me_resolution.height = 0;
    a2560me_font_size.width = 0;
    a2560me_font_size.height = 0;

    /* Disable the set_sizes call for now */
    a2560me_enable_set_sizes = 0;
    ...
    a2560me_caps.number = TXT_SCREEN_A2560ME;

    /* This screen can be nothing, sleep, or any combination of text, sprite, bitmap, and tile */
    a2560me_caps.supported_modes = TXT_MODE_TEXT | TXT_MODE_SPRITE | TXT_MODE_BITMAP | TXT_MODE_TILE | TXT_MODE_SLEEP;

    /* Resolutions supported: 1280x960 */
    a2560me_resolutions[0].width = 1280;
    a2560me_resolutions[0].height = 960;
    a2560me_caps.resolution_count = 1;
    a2560me_caps.resolutions = a2560me_resolutions;

    /* Channel B supports 8x16 fonts ONLY */
    a2560me_fonts[0].width = 8;
    a2560me_fonts[0].height = 16;
    a2560me_caps.font_size_count = 1;
    a2560me_caps.font_sizes = a2560me_fonts;

    /* Initialize the color lookup tables */
    for (i = 0; i < VKY3_B_LUT_SIZE; i++) {
        VKY3_B_TEXT_LUT_FG[i] = a2560me_lut[i];
        VKY3_B_TEXT_LUT_BG[i] = a2560me_lut[i];
    }

    /* Set the mode to text */
    txt_a2560me_set_mode(TXT_MODE_TEXT);

    /* Set the resolution */
    txt_a2560me_set_resolution(1280, 960);                  /* Default resolution is 1280x960 */
    ...
    txt_a2560me_set_region(&region);

    /* Home the cursor */
    txt_a2560me_set_xy(0, 0);
 ...
 */
short txt_a2560me_install() {
    t_txt_device device;

    device.number = TXT_SCREEN_A2560ME;
    device.name = "SCREEN";

    device.init = txt_a2560me_init;
    device.get_capabilities = txt_a2560me_get_capabilities;
    device.set_mode = txt_a2560me_set_mode;
    device.set_sizes = txt_a2560me_set_sizes;
    device.set_resolution = txt_a2560me_set_resolution;
    device.set_border = txt_a2560me_set_border;
    device.set_border_color = txt_a2560me_set_border_color;
    device.set_font = txt_a2560me_set_font;
    device.set_cursor = txt_a2560me_set_cursor;
    device.set_cursor_visible = txt_a2560me_set_cursor_visible;
    device.set_region = txt_a2560me_set_region;
    device.get_region = txt_a2560me_get_region;
    device.set_color = txt_a2560me_set_color;
    device.get_color = txt_a2560me_get_color;
    device.set_xy = txt_a2560me_set_xy;
    device.get_xy = txt_a2560me_get_xy;
    device.put = txt_a2560me_put;
    device.scroll = txt_a2560me_scroll;
    device.fill = txt_a2560me_fill;
    device.get_sizes = txt_a2560me_get_sizes;
