...
uint16_t mem_mcr_shadow = 0;           		/* A shadow register for the Master Control Register */
    ...
    if (mode & TXT_MODE_SLEEP) {
        ...
        *tvky_mstr_ctrl = mem_mcr_shadow;
        ...
        if (mode & ~(TXT_MODE_TEXT | TXT_MODE_BITMAP | TXT_MODE_SPRITE | TXT_MODE_TILE)) {
            ...
            if ((mem_mcr_shadow & (VKY_MCR_GRAPHICS | VKY_MCR_TEXT)) == (VKY_MCR_GRAPHICS | VKY_MCR_TEXT)) {
